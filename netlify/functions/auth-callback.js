  ...
  try {
    const response = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        code,
        client_id: process.env.GOOGLE_WORK_CLIENT_ID,
        client_secret: process.env.GOOGLE_WORK_CLIENT_SECRET,
        redirect_uri: process.env.GOOGLE_WORK_REDIRECT_URI,
    ...
    return {
      ...
      headers: { 'Content-Type': 'text/html' },
      body: `<html><body style="background:#0f0f1a;color:#f0f0f0;font-family:sans-serif;text-align:center;padding:60px">
        <h1>✅ Work Calendar Connected!</h1>
        <p style="color:#aaa">Copy this refresh token and add it to Netlify environment variables as <strong>GOOGLE_WORK_REFRESH_TOKEN</strong></p>
